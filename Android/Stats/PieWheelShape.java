...
public class PieWheelShape {
    ...
    private int CIRCLE_POINTS = 360;
    private float pointsForSlice;
     ...
     */
    public PieWheelShape(float r, float[] color, float portion, boolean isOffset) {

        pointsForSlice = (float) CIRCLE_POINTS * portion;
        circleRadius = r;
        circleColor = color;

        float circleCoords[] = new float[(1 + (int)pointsForSlice) * COORDS_PER_VERTEX];

        //set center coordinates (x,y,z)
        //check if shadow, for center offset
        if (isOffset){
            circleCoords[0] = .03f;
            circleCoords[1] = -.03f;
            circleCoords[2] = 0;
        ...
        int idx = 3;
        for (int i = 0; i < (int)pointsForSlice; i++){
    ...
    public void draw(float[] mvpMatrix) {
        ...
        // Draw the square
        GLES20.glDrawArrays(GLES20.GL_TRIANGLE_FAN, 0, ((int)pointsForSlice+1));
